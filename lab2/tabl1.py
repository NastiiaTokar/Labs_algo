...
while x <= b: #Цикл виконується, поки x не досягне b
    if x < 7:
        y = func1(x) #Використання першої функції
    elif 7 <= x < 8: #Перевірка умови
        y = func2(x) #Використання другої функції
    else: #Запасний варіант
        y = func3(x) #Використання третьої функції

if y is not None: #y не дорівнює None
    print ("{x:<10.4f}{y:<20.10f}")
else: #y дорівнює None
    print ("{x:<10.4}{'Немає значення':<20}")
